...
def extract_text_from_html(html):
    ...
    return soup.get_text(separator=' ', strip=True)  # 텍스트 추출 및 정리
...
def summarize_text(text):
    ...
    # 요약을 위한 프롬프트 템플릿 생성
    summary_prompt = PromptTemplate.from_template("아래의 내용을 요약해줘: {text}")
...
async def reply_message(request: Request):
    ...
    # 요약 결과 반환
    return {"response": f"{sender}에게 답장합니다: {summary}"}
